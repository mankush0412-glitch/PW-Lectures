...
# ── Debug: log ALL incoming messages so we can see in Render logs ─────────────
...
async def debug_logger(bot: AFK, m: Message):
    try:
        ...
        LOGGER.info(f"[MSG RECEIVED] uid={uid} | text={txt[:80]}")
    except Exception as e:
        LOGGER.warning(f"[DEBUG LOGGER ERROR] {e}")


# ── /ping — no auth, always replies (for testing) ─────────────────────────────

@AFK.on_message(filters.command("ping", prefixes))
async def ping(bot: AFK, m: Message):
    uid = m.from_user.id if m.from_user else 0
    is_ok = is_auth_uid(uid)
    await m.reply_text(
        f"🏓 <b>Pong!</b>\n"
        f"Your ID: <code>{uid}</code>\n"
        f"OWNER_ID: <code>{Config.OWNER_ID}</code>\n"
        f"Auth: {'✅ YES' if is_ok else '❌ NO'}",
        quote=True
    )
...
async def start(bot: AFK, m: Message):
    try:
        ...
        LOGGER.error(f"[START] is_auth error: {e}")
        await m.reply_text(f"⚠️ Error: {e}", quote=True)
        return
