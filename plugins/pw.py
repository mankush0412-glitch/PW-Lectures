...
# ── Text router — resolves waiting conversations (replaces pyromod) ───────────

@AFK.on_message(filters.private & filters.text, group=-1)
async def text_router(bot: AFK, m: Message):
    if not m.from_user:
        return
    chat_id = m.chat.id
    resolved = conv_resolve(chat_id, m)
    if resolved:
        bot.stop_propagation()


# ── Debug: log every incoming message ─────────────────────────────────────────

@AFK.on_message(filters.all, group=-999)
async def debug_logger(bot: AFK, m: Message):
    try:
        uid = m.from_user.id if m.from_user else "?"
        txt = (m.text or m.caption or "[media]")[:60]
        LOGGER.info(f"[IN] uid={uid} chat={m.chat.id} text={txt}")
    except Exception:
        pass


# ── /ping — no auth, always replies ───────────────────────────────────────────

@AFK.on_message(filters.command("ping", prefixes))
async def ping_cmd(bot: AFK, m: Message):
    uid   = m.from_user.id if m.from_user else 0
    ok    = is_auth_uid(uid)
    await m.reply_text(
        f"🏓 <b>Pong!</b>\n"
        f"Your ID: <code>{uid}</code>\n"
        f"OWNER_ID set: <code>{Config.OWNER_ID}</code>\n"
        f"Auth: {'✅ YES' if ok else '❌ NO — ask owner to add you'}",
        quote=True,
    )
...
async def pw_login(bot: AFK, m: Message):
    ...
    try:
        phone_msg = await conv_ask(bot, m.chat.id, "📞 Send phone number:", timeout=120)
    except Exception:
    ...
    await wait.edit_text("✅ <b>OTP sent!</b> Check PW app or SMS.")
    try:
        otp_msg = await conv_ask(bot, m.chat.id, "🔢 Enter OTP:", timeout=120)
    except Exception:
...
async def batch_cmd(bot: AFK, m: Message):
    if not is_auth(m):
        return
    try:
        txt = await conv_ask(bot, m.chat.id, "📋 Paste links (Name:URL format):", timeout=300)
    except Exception:
        await m.reply_text("⏰ Timeout!")
        return
...
async def single_dl(bot: AFK, m: Message):
    ...
    try:
        nm   = await conv_ask(bot, m.chat.id, "📝 File name (or send 'skip'):", timeout=60)
    ...
    if result:
        ul = Upload_to_Tg(bot, m, result, name, get_thumb(uid), Config.DOWNLOAD_DIR, sm, name)
        if result.endswith(".pdf"):
            await ul.upload_doc()
        else:
            await ul.upload_video()
    else:
        await sm.edit_text("❌ Download failed!")
...
async def help_cmd(bot: AFK, m: Message):
    ...
    await m.reply_text(
        ...
        "<b>Download:</b>\n"
        "• Send .txt file → batch download\n"
        "• /dl URL — Single download\n"
        "• /batch — Paste links\n\n"
        "<b>Settings:</b>\n"
        "• /setthumb URL — Video thumbnail\n\n"
        "<b>Admin (owner only):</b>\n"
        "• /adduser ID  /remuser ID  /users  /restart\n\n"
        "<b>Debug:</b>\n"
...
async def _show_and_download(bot, m, status, uid, links):
    ...
    summary += "\n<b>Reply:</b> index(es) e.g. <code>1 3 5</code> or <code>all</code>"

    await status.edit_text(summary)

    try:
        sel = await conv_ask(bot, m.chat.id, "✏️ Selection:", timeout=300)
    except Exception:
        await m.reply_text("⏰ Timeout!")
        return
    ...
    await m.reply_text(f"🚀 <b>Starting {len(selected)} downloads (Quality: {quality})</b>")
    ...
    for i, idx in enumerate(selected):
        ...
        try:
            ...
            if file_path == "SKIPPED":
                skipped += 1
                await _edit(sm, f"⏭ <b>Skipped (exists):</b> <code>{name}</code>")
                continue
            ...
            if file_path and os.path.isfile(file_path):
                ...
                await _edit(sm, f"❌ <b>Failed:</b> <code>{name}</code>")
                fail += 1
...
async def _process_one(uid, name, raw_url, quality, output_dir, sm):
    ...
    return await DRMDecrypt.download_and_decrypt(
        ...
        output_dir=output_dir, quality=quality, progress_cb=_progress,
    )
